...
int main(int argc, char **argv) {
    ...
    /* I am testing with an empty string*/
    
    char *str3 = ""; 
    int num_empty = num_occurrences(str3, 'a'); 
    assert(num_empty == 0);
    ...
    /*test with a sequence containing only one nucleotide type*/
    
        DNA_sequence dna_seq_3;
	strcpy(dna_seq_3.sequence, "GGGGGGGG");
	compute_nucleotide_occurrences(&dna_seq_3);
	assert(dna_seq_3.A_count == 0);
	assert(dna_seq_3.C_count == 0);
	assert(dna_seq_3.G_count == 8);
